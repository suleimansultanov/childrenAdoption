...
{
    public class CreateESPAdultController : Controller
    {
        [HttpGet]
        public ActionResult Index()
        {
            var model = new CreateESPAdultViewModel() { ActionNo = 1 };
        ...
        {
            if (SendRequest(null, "http://localhost/SourcesAPI/api/CISSA/GetESPStatus?pin=" + pin, "GET", out dynamic result, out string error))
            {
            ...
            return View("~/Views/CreateApplication2/GetStatus.cshtml");
        }
        [HttpGet]
        public ActionResult GetMSECInfo()
        {
            return View("~/Views/CreateApplication2/GetMSECInfo.cshtml", new MSECRequest());
        }
        [HttpPost]
        public ActionResult GetMSECInfo(MSECRequest request)
        {
            if (SendRequest(new
            {
                PIN = request.PIN
            },
            "http://localhost/SourcesAPI/api/CISSA/MSECDetails", "POST", out dynamic response, out string errorMessage))
            {
                if (response != null)
                {
                    var r = response;
                    request.msecData = ((JObject)r).ToObject<_msecData>();
                }
                else
                {
                    //ViewBag.Error = "Данные о браке отсутствуют";
            ...
            return View("~/Views/CreateApplication2/GetMSECInfo.cshtml", request);
        }
        ...
        static bool SendSocFundRequest(string pin, string orgName, out TempResult result, out string errorMessage)
        {
            errorMessage = "";
            result = new TempResult();
            try
            {
                string webAddr = "http://localhost/ServiceConstructor/SoapClient/SendRequest2";
                ...
                using (var streamWriter = new StreamWriter(httpWebRequest.GetRequestStream()))
                {
                    string json = Newtonsoft.Json.JsonConvert.SerializeObject(
                        new
                        {
                            ...
                            clientId = new Guid("a50e1e20-65f9-460f-b70d-b27ce1b79464"),
                            orgName
                        });
        ...
        public class TempResult
        {
            ...
            {
                public _GetWorkPeriodInfoResponse GetWorkPeriodInfoResponse { get; set; }

                public class _GetWorkPeriodInfoResponse
                {
                    ...
                    public _WorkPeriods WorkPeriods { get; set; }
                    public class _WorkPeriods
                    {
                        public _WorkPeriod[] WorkPeriod { get; set; }
                        public class _WorkPeriod
                        {
        ...
        public class CreateESPAdultViewModel
        {
            ...
            public _msecData msecData { get; set; }
